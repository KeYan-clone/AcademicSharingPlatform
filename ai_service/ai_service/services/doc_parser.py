...
# 允许没有安装 Tesseract 时跳过 OCR，避免报错
DEFAULT_TESSERACT = r"D:\Tesseract-OCR\tesseract.exe"
if shutil.which("tesseract"):
    ...
    logger.warning("Tesseract 未安装或未配置，图片型 PDF 将跳过 OCR 提取。")

class DocumentParser:
    """文档解析器，支持多种文档格式的内容提取"""
    
    SUPPORTED_EXTENSIONS = {'.pdf', '.docx', '.doc', '.txt', '.md', '.json', '.csv', '.xlsx', '.html', '.jpg', '.jpeg', '.png', '.bmp', '.tiff'}
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if not text:
                        ...
                        if pytesseract.pytesseract.tesseract_cmd:
                            ...
                            logger.warning(f"PDF {file_path} 第{page_num}页无可提取文本，且未配置Tesseract，跳过OCR")
                            empty_pages += 1
                    else:
